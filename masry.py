...
def GeT_OB():
    try:
        ...
        data = response.json()
        data['server_url'] = 'https://loginbp.ggpolarbear.com/'  # إضافة server_url المطلوب
...
def AuToUpDaTE():
    ...
    data, ob = GeT_OB()
    if data:
        url = data.get('server_url', 'https://loginbp.ggpolarbear.com/')  # استخدام server_url من البيانات أو الرابط الافتراضي
        return url, ob, version
    else:
        return 'https://loginbp.ggpolarbear.com/', None, version
